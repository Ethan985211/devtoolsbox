...
function md5Pure(string) {
  ...
  function rhex(n) { var s = '', j; for (j = 0; j <= 3; j++) { s += HEX_CHARS.charAt((n >> (j * 8 + 4)) & 0x0F) + HEX_CHARS.charAt((n >> (j * 8)) & 0x0F); } return s; }
...
function verifySign(params, sign) {
  ...
  const expectedSign = md5Pure(signStr);
...
export async function onRequestGet({ env, request }) {
  ...
  const order = await getOrderStatus(db, outTradeNo);
  if (!order) {
    return new Response('order not found', { status: 404 });
  }

  if (order.paid) {
    return new Response('success'); // Already processed
  }

  // Activate membership
  const days = PLAN_DAYS[order.plan] || 30;
  await activateMembership(db, order.username, order.plan, days, outTradeNo, order.amount);
